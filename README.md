...
Консольная читалка книг. Работает с TXT, EPUB, FB2, MOBI, AZW, AZW3, PDF, DJVU, DOCX, DOC, HTML, Markdown, RTF и книгами в формате IPFS Media (манифест `.ipfsbook`). Тёмный минималистичный интерфейс, библиотека хранится в SQLite, есть закладки, заметки к тексту и прогресс чтения.
...
Форматы без лишних зависимостей разбираются сами: TXT (кодировка определяется автоматически), EPUB, FB2 (в том числе `.fb2.zip`), HTML, Markdown, RTF, DOCX, MOBI и AZW (свой PalmDB-парсер). PDF открывается через pypdf, DJVU через djvutxt, DOC через LibreOffice или antiword. Формат `.ipfsbook` - JSON-манифест книги по схеме protocol-core (ipfs://schema/book/0.1.0): метаданные, авторы, год, язык и главы как WorkComponent с ContentBlob. Текст глав берётся из inline-поля (text/base64) или файла рядом с манифестом. Разобранные книги кэшируются в SQLite, поэтому повторное открытие не перечитывает файл.
